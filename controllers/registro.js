...
let correo
let isNumber
const flowRegistro = addKeyword(['registro', 'reg', 'uno', 'one', '1'], {sensitive: true})
    .addAnswer(['Favor de ingresar su *Número de identificador de  cliente*, este número consta de 6 dígitos.'],
        { capture: true },
    ...
    .addAnswer('Favor de proporcionar solo su(s) *Nombre(s)*', { capture: true }, (ctx, { fallBack }) => {
                nombre = ctx.body
    ...
    .addAnswer('Favor de proporcionar solo su(s) *Apellido(s)*', { capture: true }, (ctx, { fallBack }) => {
        apellido = ctx.body
        isNumber = Number(apellido)
        if (!isNaN(isNumber)) return fallBack()
        if (apellido.length < 3) return fallBack()
    })
    .addAnswer('Favor de proporcionar el *Correo electrónico* con el que se desea realizar el registro a este curso', { capture: true }, (ctx, { fallBack }) => {
        correo = ctx.body
        const regex = /\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)/
        if (correo.length < 3) return fallBack()
        if (!correo.includes('@')) return fallBack()
    })
    .addAnswer('!Te deseamos mucha suerte¡ 🥇🍀🎁', null, async (ctx, {flowDynamic}) => {
    await flowDynamic(
